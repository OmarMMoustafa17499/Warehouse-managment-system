...
const getorderId = ()=>{
    ...
    axios({
        method:"get",
        url:`http://127.0.0.1:8000/api/Order/${localStorage.getItem('orderId')}/`
    }).then((response)=>{
  ...
  if(updateOrder){
    updateOrder.addEventListener('click',()=>{
      const data = {
        ...
        axios({
            method: "put",
            url: `http://127.0.0.1:8000/api/Order/${localStorage.getItem('orderId')}/update/`,
            data: data,
        }).then((response)=>{
