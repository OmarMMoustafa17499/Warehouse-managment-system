...
let msgCar = document.getElementById('msgCar')
 ...
 const getCarId = ()=>{
    ...
    axios({
        method:"get",
        url:`http://127.0.0.1:8000/api/CarDrop/${localStorage.getItem('carId')}/`
    }).then((response)=>{
...
if(updateCar){
    updateCar.addEventListener('click',()=>{
        ...
        axios({
            method: "put",
            url: `http://127.0.0.1:8000/api/CarDrop/${localStorage.getItem('carId')}/update/`,
            data: data,
        }).then((response)=>{
           if(response.status ===200){
            window.location.replace('C:/Users/OMAR/Desktop/Omar_Web/omar/omar/carDrop.html');
           }
        }).catch((error)=> {
